...
const useAuth = () => {
  const [authenticated, setAuthenticated] = useState(() => {
    return Cookies.get("authenticated") === "true"; // Check cookie
  });

  const toggleAuthenticated = () => {
    const newAuthState = !authenticated; // Toggle the state
    setAuthenticated(newAuthState);
    Cookies.set("authenticated", newAuthState.toString(), { expires: 7 }); // Persist to cookies
  };

  return { authenticated, toggleAuthenticated };
};
